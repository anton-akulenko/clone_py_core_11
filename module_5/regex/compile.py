...
Ми використали функцію compile щоб скомпілювати попередньо патерн. Потрібно, якщо використовуємо його часто в різних місцях програми.
'''
...
string = "Нільс Бор народився в родині професора фізіології Копенгагенського університету Християна Бора " \
         ...
         " і Дженні Рафаел (1830—1902) із британської єврейської банкірської династії..."
...
# Знайти слово початку речення
pattern = re.compile(r'^[А-ЯA-Z]\w*')
result = pattern.findall(string)
print(result)

# Знайти слово наприкінці речення
